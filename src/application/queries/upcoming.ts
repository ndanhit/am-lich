...
export function getUpcomingEvents(
  ...
  const currentYearOccurrences = calculateOccurrencesForYear(
    events,
    referenceSolar.year,
  );

  // Grab occurrences for next calendar year to handle cross-boundary limits
  const nextYearOccurrences = calculateOccurrencesForYear(
    events,
    referenceSolar.year + 1,
  );

  const allCandidateOccurrences = [
    ...currentYearOccurrences,
    ...nextYearOccurrences,
  ];
  ...
  const validUpcoming = allCandidateOccurrences
    .map((occurrence) => {
      ...
      const daysUntil = Math.round(
        (occTimestamp - referenceTimestamp) / (1000 * 60 * 60 * 24),
      );
